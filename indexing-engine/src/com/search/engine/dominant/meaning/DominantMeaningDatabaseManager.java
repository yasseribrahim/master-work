 ...
 * @author interactive
 */
public class DominantMeaningDatabaseManager {

    private DatasetManager dataset;
    private CustomXmlIndexing indexing;
    private IndexingUtil util;
    private Exporter exporter;
    private IndexReader reader;
    private Tree<Bag> tree;

    public static DominantMeaningDatabaseManager getManager(String workingDirectory, String datasetDirectory) {
        return new DominantMeaningDatabaseManager(workingDirectory, datasetDirectory);
    }

    private DominantMeaningDatabaseManager(String workingDirectory, String datasetDirectory) {
    ...
    public void setReader(String name) {
        try {
            this.setReader(getUtil().getIndexReader(name));
        } catch (IOException ex) {
            System.err.println(ex);
        }
    }

    public final boolean buildModel(float percentage) {
        try {
            getIndexing().buildIndex(percentage);
            calculateAndExport();
        } catch (Exception ex) {
            System.err.println(ex);
            return false;
    ...
    public int calculateMaximumFrequenceConcept(String name) throws IOException {
        TermDocs docs = getReader().termDocs(new Term(Naming.FIELD_CONTENT, name));
    ...
    public int calculateMaximumFrequenceTerm() throws Exception {
        TermEnum terms = getReader().terms();
        TermDocs docuemts;

        int maximum = 0;
        while (terms.next()) {
            docuemts = getReader().termDocs(terms.term());
    ...
    public Node<Bag> calculate(Node<Bag> root, String concept) throws Exception {
        TermEnum terms = getReader().terms();
        TermDocs documents;
        int numberDocuments = getReader().numDocs();
        ...
        String path, url;
        while (terms.next()) {
            documents = getReader().termDocs(terms.term());
            documentsList = new ArrayList<>();

            summation = 0.0d;
            while (documents.next()) {
                path = getReader().document(documents.doc()).get(Naming.FIELD_PATH);
                url = getUtil().getUrl(path);
    ...
    public void calculateAndExport(String concept) throws Exception {
        ...
        getExporter().exporter(node, getDataset().getOutputDirectory() + FileManager.FILE_SEPARATOR /*+ node.getChildrens().get(0).getValue().getWord() + "_"*/ + concept + ".xml");
    }

    public Tree<Bag> calculate() throws Exception {
        setTree(new Tree<>(new Bag("root", 0), null, new DefaultNodeVisitor<>()));

        for (DatasetManager.Data data : getDataset().getDatas()) {
            setReader(data.getName());
            calculate(getTree().getRoot(), data.getName());
        }
        return getTree();
    }
...
//        String datasetDir = "/home/interactive/Desktop/Master Work/Dataset/description";
        String workingDir = "/home/interactive/Desktop/Master Work/indexing-engine/output" + FileManager.FILE_SEPARATOR;
        String datasetDir = "/home/interactive/Desktop/Master Work/indexing-engine/dataset";
...
//        validateTree(workingDir, datasetDir);
    ...
    public static void validateTree(String workingDir, String datasetDir) throws Exception {
        DominantMeaningDatabaseManager manager = getManager(workingDir, datasetDir);
        ...
        String[] words, words1;
        int counter;
        for (DatasetManager.Data data : manager.getDataset().getDatas()) {
            ...
            for (DatasetManager.Data data1 : manager.getDataset().getDatas()) {
    ...
    public static void getResults(String workingDir, String datasetDir) throws Exception {
        ...
        for (DatasetManager.Data data : manager.getDataset().getDatas()) {
            reader = manager.getUtil().getIndexReader(data.getName());
            searcher = new IndexSearcher(reader);
            tree = manager.getExporter().importer(workingDir + "/output/" + data.getName() + ".xml");
            ...
            for (int i = 5; i <= 100; i += 5) {
                try {
                    ...
                    out.append("Query Size: ").append(queryString.split(" ").length).append(", Hit: ").append(hit).append(", Expected Result: ").append(data.getFiles().length).append(", Retrieved Result: ").append(documents.length).append("\n");
    ...
    public static void getResults2(String workingDir, String datasetDir) throws Exception {
        DominantMeaningDatabaseManager manager = getManager(workingDir, datasetDir);
        manager.buildModel(1);
        IndexReader reader = manager.getUtil().getIndexReader();
...
//        manager.buildModel();
    ...
    public void setDataset(DatasetManager dataset) {
        this.dataset = dataset;
    }

    public CustomXmlIndexing getIndexing() {
        return indexing;
    }

    public void setIndexing(CustomXmlIndexing indexing) {
        this.indexing = indexing;
    }

    public IndexingUtil getUtil() {
        return util;
    }

    public void setUtil(IndexingUtil util) {
        this.util = util;
    }
    ...
    public void setTree(Tree<Bag> tree) {
        this.tree = tree;
    }
}
